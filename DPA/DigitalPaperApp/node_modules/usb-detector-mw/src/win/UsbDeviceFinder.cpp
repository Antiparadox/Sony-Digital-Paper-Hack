...
void UsbDeviceFinder::FindUsbDevices(
	...
	tstring sTargetDevice = Utility::ToUpper(rTargetDevice);
	...
	sDeviceId = Utility::ToUpper(sDeviceId);
	...
	if (spParentDevice
		...
		if (cResult == CR_SUCCESS) {
			FindUsbDevices(sTargetDevice, ulDevInstChild, spDevice, rvDeviceList);
		}
		...
		if (cResult == CR_SUCCESS) {
			FindUsbDevices(sTargetDevice, ulDevInstSibling, spDevice, rvDeviceList);
		}
	}
	else if (!sDeviceId.empty()
		&& ::_tcsicmp(sTargetDevice.c_str(), sDeviceId.substr(0, sTargetDevice.length()).c_str()) == 0) {
		...
		cResult = ::CM_Get_Child(&ulDevInstChild, ulDevInst, 0);
		if (cResult == CR_SUCCESS) {
			FindUsbDevices(sTargetDevice, ulDevInstChild, spTargetDevice, rvDeviceList);
		}
		...
		cResult = ::CM_Get_Sibling(&ulDevInstSibling, ulDevInst, 0);
		if (cResult == CR_SUCCESS) {
			FindUsbDevices(sTargetDevice, ulDevInstSibling, nullptr, rvDeviceList);
		}
	}
	else {
		// 子ノードを探す
		...
		cResult = ::CM_Get_Child(&ulDevInstChild, ulDevInst, 0);
		if (cResult == CR_SUCCESS) {
			FindUsbDevices(sTargetDevice, ulDevInstChild, nullptr, rvDeviceList);
		}
