...
void EIO_Open(uv_work_t* req) {
  ...
  // 
  // [備考]
  // 複数のアカウントから全く同時に::CreateEvent()を呼び出して、名前付きの
  // イベントを作成した場合には、ERROR_ALREADY_EXISTS が発生せずに作成に
  // 成功してしまう。
  //
  // [環境]
  ...
  const ULONG tid = ::GetCurrentThreadId();
  const ULONG wait_time = (pid + tid) % 100;
  ::Sleep(wait_time);
  // <-
